...
def main(Network):
    ...
    if(Network == "ResNet18"):
        ...
        #model, train_det_accs, train_cls_accs, val_det_accs, val_cls_accs = train_model(model, train_loader, val_loader, num_epochs=5, lr=0.001)
        # Load the trained model
        model = KITTIObjectDetector(num_classes=9)
        model.load_state_dict(torch.load("resnet18_kitti.pth", map_location=device, weights_only=True))  # Update with correct checkpoint path
        model.to(device)
        model.eval()
        ...
        utils.predict_and_visualize_all(model, test_images)
